...
GLuint vao1,vao2, points_vbo, colors_vbo, modelID, squarepoints_vbo, squarecolor_vbo;
...
void init(void)
{
	...
	squarecolor_vbo = 0;
	glGenBuffers(1, &squarecolor_vbo);
	glBindBuffer(GL_ARRAY_BUFFER, squarecolor_vbo);
...
void display(void)
{
	...
	glBindVertexArray(vao1);
	angle = angle + 0.5f;
	transformObject(glm::vec3(.25f,.25f,1.0f), Z_AXIS, angle, glm::vec3(-0.5f, 0.5f, 0.0f));
	...
	transformObject(glm::vec3(.25f, .25f, 1.0f), Z_AXIS, angle, glm::vec3(0.0f, 0.0f, 0.0f));
	glDrawArrays(GL_LINE_LOOP, 0, 3); // GL_LINE_LOOP works too!

	glBindVertexArray(vao2);
	transformObject(glm::vec3(.5f, .5f, .5f), Z_AXIS, 0, glm::vec3(-0.5f, -0.5f, 0.0f));
	glDrawArrays(GL_LINE_LOOP, 0, 4); // GL_LINE_LOOP works too!
